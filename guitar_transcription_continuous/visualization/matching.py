...
def plot_association(pitch, interval, times, profile, contours, color='k', fig=None):
    """
    ...
    profile : InstrumentProfile
      Details about instrument profile
    ...
    for contour in contours:
        ...
        fig = tools.plot_pitch_list(*contour.get_pitch_list(times), point_size=10,
...
def plot_note_contour_associations(notes, times, profile, grouping, primary_color_loop=None,
    ...
    for n in range(len(pitches)):
        ...
        fig = plot_association(pitch, interval, times, profile, grouping[n], color, fig)
    ...
    for k in np.setdiff1d(list(grouping.keys()), np.arange(len(pitches))):
        ...
        fig = plot_association(pitch, interval, times, profile, grouping[k], color, fig)
...
if __name__ == '__main__':
    ...
    for track in tablature_data:
        ...
        # Unpack the collapsed pitch contours
        times, pitch_list = all_contours
